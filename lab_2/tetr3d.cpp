...
public:
    ...
    void move(double tau, unsigned int step) {
        ...
        double kx = M_PI/10, ky = M_PI/16;
        vx = y;
        vy = -x;
        vz = 2*(z < 10)*(10-z)*sin(kx*(x*cos(tau*step) - y*sin(tau*step)))*sin(ky*(x*sin(tau*step) + y*cos(tau*step)))*cos(20*step*tau) - 15*cos(40*step*tau);
    }

    void update_smth(double tau, unsigned int step) {
        double kx = 0.1*M_PI,ky = M_PI/16, kz = M_PI/10;
        smth = (z-10)*sin(kx*x)*sin(ky*y)*sin(kz*z - 20*step*tau)*sin(20*step*tau);
    }
};
...
public:
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "shiba-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("shiba");

    // Считаем STL
    try {
        gmsh::merge("../Shiba.stl");
    } catch(...) {
    ...
    for(unsigned int step = 1; step < 600; step++) {
