...
int main(int argc, char* argv[])
{
...

  {
    ...
    auto mesh = std::make_shared<mesh::Mesh>(
        mesh::create_box(MPI_COMM_WORLD, {{{-3.0, 0.0, 0.0}, {3.0, 1.0, 1.0}}},
                         {60, 10, 10}, mesh::CellType::tetrahedron,
    ...
    auto u_rotation_2 = std::make_shared<fem::Function<T>>(V);
    u_rotation_2->interpolate(
        [](auto x) -> std::pair<std::vector<T>, std::vector<std::size_t>>
        {
          constexpr double scale = 0.005;
    ...
    auto bdofs_right = fem::locate_dofs_geometrical(
        ...
        [](auto x)
        {
          ...
          for (std::size_t p = 0; p < x.extent(1); ++p)
          {
            if (std::abs(x(0, p) - 3) < eps)
              marker[p] = true;
          }
          return marker;
        });
    auto bdofs_right_2 = fem::locate_dofs_geometrical(
        {*V},
        [](auto x)
        {
          constexpr double eps = 1.0e-8;
    ...
    auto bcs = std::vector{
        ...
        std::make_shared<const fem::DirichletBC<T>>(u_rotation_2, bdofs_right_2)};
